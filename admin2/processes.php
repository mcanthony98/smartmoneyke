...
if(isset($_POST["login"])){
    $_SESSION['chid'] = "1";
    $_SESSION['chname'] = "Anne Admin";
    header("location:  dashboard.php");
}
elseif(isset($_POST["new-service"])){
    $name = mysqli_real_escape_string($conn, $_POST["name"]);
    $hdate = mysqli_real_escape_string($conn, $_POST["dateheld"]);
    ...
    $insert = "INSERT INTO service (name, slag, short_desc, long_desc, date_held, image, date_created) VALUES ('$name', '$slag', '$sdesc', '$ldesc', '$hdate', '$new', '$date')";
    
    if ($conn->query($insert)===TRUE){
        $pk = $conn->insert_id;
          
          $_SESSION["success"] = "New Tournament Added Sucessfully.";
...
elseif(isset($_POST["edit-service"])){
    ...
    $insert = "UPDATE `service` SET `name`='$name',`slag`='$slag',`short_desc`='$sdesc', `date_held`='$hdate' ,`long_desc`='$ldesc', `status`='$status' WHERE service_id='$id'";
    
    if ($conn->query($insert)===TRUE){
        
          
          $_SESSION["success"] = "Tournament Updated Sucessfully.";
